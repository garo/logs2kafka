...
function handle(event) {
	this.doSendMessage(this.topic, JSON.stringify(event), 0);
}

function doSendMessage(topic, message, retries) {
  var self = this;
  ...
  this.producer.send({
    ...
    if (result.error) {
      ...
      if (retries < self.maxRetries) {
        ...
        setTimeout(doSendMessage.bind(self, topic, message, retries), self.retryInterval);
      } else {
